...
const BitcoinWealthComparisonChart: React.FC<BitcoinWealthComparisonChartProps> = ({
  ...
  const comparisonData = useMemo(() => {
    ...
    for (let year = 1; year <= loanTermYears; year++) {
      ...
      const homeEquity = homeEquityEntry ? (homeEquityEntry.principalPaid || 0) : 0;
  ...
  const CustomTooltip: React.FC<TooltipProps<ValueType, NameType>> = ({ active, payload }) => {
    ...
    return (
      <div className="bg-white dark:bg-gray-800 p-4 rounded shadow-lg border-2 border-orange-300 dark:border-orange-500">
        ...
        <p className={`font-semibold text-sm ${isBitcoin ? 'text-orange-600' : 'text-blue-600'}`}>
    ...
    <div className="w-full space-y-6">
      ...
      <div className="bg-blue-50 dark:bg-blue-900/20 border-l-4 border-blue-500 p-4 rounded">
        <p className="text-sm text-gray-700 dark:text-gray-300">
          <strong>Scenario:</strong> What if you invested your ${downPayment.toLocaleString()} down payment in Bitcoin instead? This chart compares:
        </p>
        <ul className="text-sm text-gray-700 dark:text-gray-300 mt-2 ml-4 space-y-1">
          <li>🏠 <strong>Mortgage Equity:</strong> Principal you pay down (mortgage wealth)</li>
          <li>₿ <strong>Bitcoin Value:</strong> Down payment invested in Bitcoin at selected growth rate</li>
        </ul>
      </div>
      ...
      <div className="bg-gradient-to-r from-orange-50 to-red-50 dark:from-orange-900/20 dark:to-red-900/20 border border-orange-300 dark:border-orange-500/50 p-5 rounded-lg">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="text-right">
            <span className="text-2xl font-black text-orange-600 dark:text-orange-400">{bitcoinCAGR}%</span>
      ...
      <div className={`p-6 rounded-lg border-2 ${bitcoinWins ? 'bg-orange-50 dark:bg-orange-900/20 border-orange-500' : 'bg-blue-50 dark:bg-blue-900/20 border-blue-500'}`}>
        <p className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">💡 After {loanTermYears} years:</p>
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-white dark:bg-gray-800 p-4 rounded border border-blue-300 dark:border-blue-500/50">
            <p className="text-xs text-gray-600 dark:text-gray-400 mb-1">🏠 Mortgage Equity</p>
            <p className="text-xl font-bold text-blue-600">{formatCurrency(equityFinal)}</p>
          </div>
          <div className="bg-white dark:bg-gray-800 p-4 rounded border border-orange-300 dark:border-orange-500/50">
            <p className="text-xs text-gray-600 dark:text-gray-400 mb-1">₿ Bitcoin Value</p>
            <p className="text-xl font-bold text-orange-600">{formatCurrency(bitcoinFinal)}</p>
          </div>
        </div>
        <div className="mt-4 pt-4 border-t border-gray-300 dark:border-gray-600">
          <p className={`font-bold ${bitcoinWins ? 'text-orange-600' : 'text-blue-600'}`}>
            {bitcoinWins ? '₿ Bitcoin' : '🏠 Home'} ahead by: <span className="text-2xl font-black">{formatCurrency(difference)}</span>
          </p>
        </div>
      </div>
      ...
      <p className="text-xs text-gray-600 dark:text-gray-400 italic bg-gray-100 dark:bg-gray-800 p-3 rounded border border-gray-300 dark:border-gray-700">
        ⚠️ Educational only. Bitcoin is volatile. Shows principal paydown only (no home appreciation). Not financial advice.
      </p>
    </div>
  );
};
