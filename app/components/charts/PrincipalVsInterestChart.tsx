    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-white dark:bg-gray-800 p-4 rounded shadow">
          ...
          <p className="text-green-600">{`Interest: ${formatYAxisTick(payload[2]?.value || 0)}`}</p>
          <p className="text-blue-600">{`Principal: ${formatYAxisTick(payload[1]?.value || 0)}`}</p>
          <p className="text-purple-600">{`Extra Payments: ${formatYAxisTick(payload[0]?.value || 0)}`}</p>
        </div>
      );
    }
    return null;
